...
## 2026-07-13 追補（cycle 43: ロルの定理）

本追補は cycle 43（`RESEARCH_LOG.md`）の結果を反映し、上記「完成へ向けた9項目
ロードマップ」項目4の状態と「現時点の実務的評価」のうち一件のみを更新する
（他の項目・本文・これより上の追補は変更しない）。

`incidence-theory/IncidenceTheory/Reals.lean` に、直前の追補（cycle 42・極値
定理）が予告したとおりの経路で、フェルマーの内点極値定理
（`realHasDerivativeAt_zero_of_interior_max` / `realHasDerivativeAt_zero_of_interior_min`
— 閉区間 `[lower, upper]` 上で大域的極値を内点で取る点では、その点での導関数は
存在すれば零）と、それを土台にしたロルの定理そのもの
（`real_rolle` — `lower < upper`・閉区間全体で連続かつ微分可能・両端点で同値
という前提から、内点に導関数零の点が存在することを結論する）を sorry-free に
証明した。証明は非負実数からの正有理数抽出・積の符号・除算の符号といった、
この既存コードベースが未だ持っていなかった実数の符号代数の小さな一群
（8個の補題）を新規に構築し、それを `realMul_differenceQuotient` の既存の
代数恒等式（`増分 × 商 = f(点+増分) - f(点)`）と組み合わせて差分商の符号を
読み取る形で行った。ロルの定理自体は、極値定理（cycle 42）が与える最大点・
最小点のうち少なくとも一方が端点の共有値と異なればそれが内点でありフェルマーの
定理がそのまま適用でき、両方とも端点の共有値と一致する場合は関数が区間全体で
定数であることが従うため、新規に構成した任意の内点がトリビアルにその点自身の
極値になりフェルマーの定理が同様に適用できる、という場合分けで完結する。
`#print axioms`（`propext`・`Classical.choice`・`Quot.sound` のみ、既存プロファイルと
同一）・`lake build` 全体・`verify.sh`（clean rebuild・example 実行・repo 全体の
未証明宣言 grep）をすべて確認済み。

これにより、9項目ロードマップの項目4「Rolle・平均値定理」は
**未完 → 部分完了** に更新する: 「極値定理、内点極値での導関数零、補助関数構成」
という三段のうち最初の二段（極値定理は cycle 42 で既に完了、内点極値での導関数零
はフェルマーの定理として本cycleで完了）に加えて、ロルの定理（割線を引かない
特殊ケース `f(a) = f(b)`）そのものも本cycleで完了した。残るのは一般の平均値定理
（割線を引いた補助関数 `h(x) = f(x) - f(a) - ((f(b)-f(a))/(b-a))(x-a)` を構成し
ロルの定理を `h` に適用する最終段）のみで、これは意図的に本cycleの範囲外とした
（cycle 41 以前からの既存の affine 微分規則・減法の微分規則がそのまま使えるため、
次cycleでの完了が現実的な見込みとして `RESEARCH_LOG.md` cycle 43 に記録済み）。

現時点の実務的評価のうち「構成実数と初等解析」を約85%から約88%に更新する
