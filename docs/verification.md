...
| Finite well-founded incidence | A bundled two-element, nonempty-boundary structure-satisfaction certificate (`root → leaf`), decreasing rank, gluing spec, finite `∂² = 0` |
...
| Internal logic | Propositional formulas over incidence atoms, natural deduction, weakening, substitution, cut, functorial/reflecting proof transport under atom maps, Kripke pullback with exact forcing preservation and equivalence transport, Kripke persistence/soundness, relative empty-context consistency, concrete two-world countermodels showing excluded middle nonderivable/invalid (including the actual `FiniteIncidence.root` atom) while its double-negation is derivable, explicit-enumeration infinite Lindenbaum chains whose limits are closed consistent prime theories extending every consistent finite context, finite-support relative chains over arbitrary prime sources, transitive finite subformula closure/decision, and canonical/ordinary same-universe Kripke completeness; a checked triangular traversal constructs the requisite recurrent schedule from every supplied exhaustive formula enumeration, including concrete Bool and `FiniteIncidence` (`leaf`/`root`) languages |
