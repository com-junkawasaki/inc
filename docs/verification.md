...
| Triangle model | Six incidence-matrix entries, three Laplacian entries, its finite `∂² = 0` calculation, and a nontrivial bisimulation quotient in which all three edge representatives agree while an edge class is separated from the empty-boundary vertex class |
