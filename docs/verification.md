...
| Triangle model | Six incidence-matrix entries, three Laplacian entries, its finite `∂² = 0` calculation, zero vertex-row sums and trace `6` of the vertex Laplacian, two independent eigenvectors of eigenvalue `3` plus the constant kernel vector, and a nontrivial bisimulation quotient in which all three edge representatives agree while an edge class is separated from the empty-boundary vertex class |
