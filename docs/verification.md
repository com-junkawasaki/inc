...
| Triangle model | Six incidence-matrix entries, three Laplacian entries, its finite `∂² = 0` calculation, checked boundary row-balance and full Laplacian row/column sums, zero vertex-row sums and trace `6` of the vertex Laplacian, two independent eigenvectors of eigenvalue `3` plus a full-index constant kernel vector derived from balance, and a nontrivial bisimulation quotient in which all three edge representatives agree while an edge class is separated from the empty-boundary vertex class |
...
| Finite set-fragment model | A bundled HF recursive-quotient witness for empty, exact pairing/product, product zero/monotonicity/distribution and singleton-powerset bridge, powerset with `⋃P(s)=s` and exact powerset-image join/LUB law, respectful separation, exact big union, extensionality, and foundation |
