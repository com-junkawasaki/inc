...
| Linear data | Boundary matrices and Laplacians computed from boundaries; pointwise boundary equality preserves both, every derived `BᵀB` Laplacian is symmetric with nonnegative diagonal entries, append/cons decomposition, exact diagonal increments, and monotonicity under row extension |
| Triangle model | Six incidence-matrix entries, three Laplacian entries, its finite `∂² = 0` calculation, zero vertex-row sums and trace `6` of the vertex Laplacian, and a nontrivial bisimulation quotient in which all three edge representatives agree while an edge class is separated from the empty-boundary vertex class |
...
| Finite linear completeness | On the nonempty finite model, equal boundary-matrix/Laplacian observations imply bisimilarity; the exact Laplacian is an idempotent projection with trace `1`, root kernel direction, and observed leaf coordinate |
...
| Finite set-fragment model | A bundled HF recursive-quotient witness for empty, exact pairing, exact Cartesian product, respectful separation, exact big union, extensionality, and foundation |
| Natural-number reconstruction | Injective von Neumann naturals in the recursive quotient, with rank `n`, exact membership `m ∈ n ↔ m < n`, internal inclusion exactly `m ≤ n`, equality/trichotomy, successor `n + 1 = n ∪ {n}`, `⋃(n+1)=n`, and binary union `max(m,n)` with associative/commutative/idempotent laws |
